...
1. **Inteligencia Territorial y Algoritmo AquaEngine:** Mapeo SIG que consulta altimetría satelital (API Open-Meteo, DEM SRTM 30m) y, cuando esa consulta falla, cae a un estimador local heurístico de respaldo. Sobre esa cota, un motor de reglas (no un modelo hidrogeológico calibrado con pruebas de bombeo) clasifica el predio como fondo de cañada aluvial con nivel somero accesible (4–12m) o cumbre/ladera donde la perforación somera se rechaza de inmediato. Es una heurística de primera aproximación explícitamente etiquetada como "estimación" en la interfaz — no un dato de campo.
...
4. **Red Centinela y Telemetría IoT Regulada (arquitectura propuesta, aún sin firmware en el repo):** Nodos ESP32 con sensores de conductividad (TDS), nivel estático/dinámico ultrasónico, caudalímetro YF-S201 y electroválvula de corte preventivo, diseñados para evitar el abatimiento local del acuífero y compartir datos abiertos con la autoridad. El repositorio hoy contiene el diseño y el dashboard que simula sus lecturas, no código de firmware ejecutable — pendiente antes de afirmar ante el jurado que el hardware ya opera.
...
| **Decreto de Veda** | **15 de mayo de 1965** (DOF, Veda Tipo III) | Prohíbe nuevas concesiones de explotación masiva. El proyecto **no asume autorización automática para extracción doméstica**: opera bajo el principio explícito en `PLAN_MAESTRO.md` de "no proponemos operar sin autorización" — el MVP es datos, muestreo legal y consulta escrita (Art. 8º), no perforación de facto. Además, la Ley de Aguas Nacionales fue reformada por el DOF el 11/dic/2025 (fuente S17 del Plan Maestro); cualquier lectura del Art. 35 debe verificarse contra el texto vigente post-reforma, no solo contra el decreto de 1965. |
...
| **Química del Agua** | **850 a 7,000 μS/cm** (600–4,900 ppm TDS) | Supera límites permisibles en Na, Cl, SO4, Mn, Fe y B. Justifica el tren 80/20 con Ósmosis Inversa; pendiente validar con ficha técnica del fabricante la tasa de rechazo real de una membrana TFC residencial frente a boro a pH neutro, que suele ser más difícil de remover que otros iones. |
...
1. `index.html`: Aplicación SPA completa construida en Vanilla HTML5/CSS3/JavaScript con:
   ...
   - **Altímetro Satelital en Tiempo Real:** Llama a la API pública Open-Meteo (elevación derivada de SRTM 30m) con rechazo automático para cumbres (>220 msnm) y zonas de falla activa (Sánchez Taboada); si la llamada falla, usa un estimador local de respaldo basado en distancia a hitos geográficos conocidos (no es DEM real, es una aproximación declarada como tal en la UI).
   - **Canvas Estratigráfico 2D:** Renderizado dinámico de la columna de suelo, sello sanitario de bentonita (2.0–2.5m), gravas y ranurado de PVC — ilustrativo, no basado en corte litológico de campo por punto.
   - **Generador de Oficio Legal en 1 Clic:** Redacta una solicitud fundamentada en el **derecho de petición (Art. 8º Constitucional)** y el Art. 4º (derecho humano al agua), pidiendo a CONAGUA orientación/consulta — no es un instrumento que autorice perforación, y así debe presentarse.
   - **Calculadora 80/20 y Unit Economics:** Simulación de consumo, dimensionamiento de líneas y retorno de inversión ($11.5 meses vs pipas) con capex de referencia fijo; falta conectar la calculadora a inputs reales de campo para que sea dinámica.
2. `PLAN_MAESTRO.md`: Documento maestro de 800 líneas con el marco hidrogeológico, económico, regulatorio, árbol de decisiones (Gates 0 a 5) y análisis de riesgos — es la fuente más cautelosa y precisa del repo en materia legal; este dossier debe ser consistente con su tono, no con lenguaje de certeza jurídica.
...
4. `docs/manual_aquadrill_v1_protegido.zip`: Manual confidencial de perforación de 14 etapas cifrado en AES-256, avalado por Antigravity y blindado en `.gitignore` (la contraseña se comparte por canal separado, nunca en este documento).
...
2. **Defensa Jurídica y Regulatoria en México:**
   - ¿La postura del proyecto (no operar sin autorización; MVP = datos + consulta escrita, per `PLAN_MAESTRO.md`) es defendible frente a CONAGUA, incluso sin invocar un "permiso implícito" del Art. 35 LAN?
   - ¿Cómo afecta a esta lectura la reforma del DOF del 11/dic/2025 a la Ley de Aguas Nacionales? ¿El Art. 35 sigue vigente en los mismos términos?
   - ¿El generador de oficio por Art. 8º Constitucional, presentado como solicitud de consulta y no como autorización, reduce razonablemente el riesgo de percepción de clandestinidad?
